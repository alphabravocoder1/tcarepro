...
# ... existing requires and class definition remain unchanged ...

def index
  # Fetch all contact trackings only for this sender
  ...
  if params[:q]&.dig(:contact_trackings_id_in).present?
    ...
    # Pick latest contact_tracking per customer *for this sender only*
    latest_ids = base_contact_trackings.select('MAX(id) as id').group(:customer_id)
    filtered_ids = base_contact_trackings.where(id: latest_ids).where(status: status).pluck(:customer_id)

    @q = Customer.where(id: filtered_ids).ransack(params[:q])
    @customers = if status == '自動送信予定'
                   @q.result.distinct
                      .joins(:contact_trackings)
                      .where(contact_trackings: { sender_id: @sender.id, status: status })
                      .order('contact_trackings.updated_at DESC')
                      .page(params[:page]).per(30)
                 else
                   @q.result.distinct.order(:id).page(params[:page]).per(30)
                 end

  elsif params[:q]&.dig(:contact_tracking_id_null) == 'true'
    contacted_ids = base_contact_trackings.select(:customer_id)
    @q = Customer.where.not(id: contacted_ids).ransack(params[:q])
  ...
  # Load contact_trackings only for visible customers to prevent N+1
  visible_ids = @customers.pluck(:id)
  @contact_trackings_hash = base_contact_trackings
                            .where(customer_id: visible_ids)
